        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @endif
                    <h1 class="text-center"> {{ $title ?? 'Cadastrar' }} </h1>
                    ...
                    <form name="formCad" id="formCad" method="post" action="{{ route('products.store') }}" class="mt-5 space-y-5">
                        ...
                        <div>
                            <x-input-label for="imageUpload" :value="__('Image')" />
                            <input id="imageUpload" name="image" type="file" class="mt-1 block w-full form-control" :value="old('image')"  accept=".png, .jpeg , .jpg" onchange="previewImage(this)"/>
                                @error('image') <span class="text-danger">{{$message}}</span> @enderror
                        </div>


                        <div class="avatar-preview">
                            <div id="imagePreview" style="background-image: " {{ route('img/logo.logo.png') }}></div>
                        </div>
