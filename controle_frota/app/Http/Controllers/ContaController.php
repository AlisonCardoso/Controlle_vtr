...
class ContaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $contas = Conta::when($request->has('nome'), function ($whenQuery) use ($request) {
            ...
            ->when($request->filled('data_inicio'), function ($whenQuery) use ($request) {
                $whenQuery->where('vencimento', '>=', Carbon::parse($request->data_inicio)->format('Y-m-d'));
            })
            ->when($request->filled('data_fim'), function ($whenQuery) use ($request) {
                $whenQuery->where('vencimento', '<=', Carbon::parse($request->data_fim)->format('Y-m-d'));
            })
            ->with('situacaoConta')
    ...
    public function create()
    {
        ...
        return view('contas.create',compact('situacoesContas'));
